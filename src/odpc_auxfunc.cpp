...
// [[Rcpp::export]]
arma::field<arma::mat> wrap_odpc_priv(const arma::mat & Z,
                                       const int & k1,
                                       const int & k2,
                                       const arma::vec & f_ini,
                                       const bool & passf_ini,
  ...
  k2_vec(0) = k2;
  mse_vec(0) = mse;
  arma::field<arma::mat> pre_ret(6, 1);
  pre_ret(0, 0) = alpha;
  pre_ret(1, 0) = B;
  pre_ret(2, 0) = k2_vec;
  pre_ret(3, 0) = mse_vec;
...
arma::field<arma::field<arma::mat> > roll_odpc(const arma::field<arma::mat> & data_field,
  ...
  for (int ind=0; ind < window_size; ind++){
    output(ind, 0) = wrap_odpc_priv(data_field(ind, 0), k, k, nothing, false, tol, niter_max, method);
  }
  # pragma omp barrier
  return(output);
}